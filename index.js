    ...
    for (var i = 0; i < groupedPostings.length; i++) {
      ...
      for (j = 0; j < groupedPostings[i].teams.length; j++) {
        for (var k = 0; k < groupedPostings[i].teams[j].postings.length; k++) {
          const tempPosting = {}
          tempPosting.teamTitle = groupedPostings[i].teams[j].teamTitle
          tempPosting.teamTitleText = sanitizeForHTML(groupedPostings[i].teams[j].teamTitle)
          tempPosting.departmentTitle = groupedPostings[i].departmentTitle
          tempPosting.categoryTeam = groupedPostings[i].teams[j].postings[k].categories.team
          tempPosting.categoryLocation = groupedPostings[i].teams[j].postings[k].categories.location
          tempPosting.categoryCommitment = groupedPostings[i].teams[j].postings[k].categories.commitment
          tempPosting.hostedUrl = groupedPostings[i].teams[j].postings[k].hostedUrl
          tempPosting.postingText = sanitizeForHTML(groupedPostings[i].teams[j].postings[k].text)
          tempPosting.categoryLocationText = (sanitizeForHTML(groupedPostings[i].teams[j].postings[k].categories.location) || '')
          postings.push(tempPosting)
        }
      }

      postings.sort((a, b) => {
        if (a.postingText < b.postingText) return -1;
        if (a.postingText > b.postingText) return 1;
        return 0;
      });
      ...
      for (let i = 0; i < postings.length; i++) {
         ...
         content += '<li class="lever-job" data-department="' + posting.departmentTitle +'" data-team="' + posting.categoryTeam + '" data-location="' + posting.categoryLocation + '"data-work-type="' + posting.categoryCommitment + '">' +
          '<a class="lever-job-title" href="' + posting.hostedUrl + '">' + posting.postingText + '</a><span class="lever-job-tag">' + posting.categoryLocationText + '</span></li>';
